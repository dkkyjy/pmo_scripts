#!/usr/bin/env python3
"""Batch-submit stats_trigger.py by auto-discovering Trigger merged YAML.

This script scans Trigger merged YAML files and submits commands in the form:
    python stats/stats_trigger.py ../Reco_Dir/Trigger_YYYYMMDD_RUNxxx_merged.yaml
"""
...
TRIGGER_FILE_PATTERN = re.compile(
    r"^Trigger_(?P<date>\d{8})_RUN(?P<run>\d+)_merged\.yaml$"
)
...
def build_arg_parser() -> argparse.ArgumentParser:
    """Build command-line parser for batch stats_trigger submission."""
    parser = argparse.ArgumentParser(
        description=(
            "Batch-submit stats/stats_trigger.py by discovering "
            "Trigger_YYYYMMDD_RUNxxx_merged.yaml."
        )
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help="Directory containing Trigger_*_merged.yaml files.",
    )
    parser.add_argument(
        "--python",
        default=sys.executable,
        help="Python executable used to run stats_trigger.py.",
    )
    parser.add_argument(
        "--script",
        default="stats/stats_trigger.py",
        help="Path to stats_trigger.py.",
    )
    parser.add_argument(
        "--avg-window",
        type=int,
        default=None,
        help="Forward --avg-window to child command.",
    )
    parser.add_argument(
        "--topn",
        type=int,
        default=None,
        help="Forward --topn to child command.",
    )
    parser.add_argument(
        "--start-datetime",
        default=None,
...
def discover_targets(reco_dir: Path, date_text: Optional[str] = None) -> List[RunTarget]:
    ...
    unique_pairs = set()
    for file_path in sorted(reco_dir.glob("Trigger_*_merged.yaml")):
...
def build_yaml_path(reco_dir: Path, target: RunTarget) -> Path:
    ...
    return reco_dir / (
        f"Trigger_{target.date_text}_RUN{target.run_number}_merged.yaml"
    )


def build_child_command(args: argparse.Namespace, yaml_path: Path) -> List[str]:
    """Build child command list for stats_trigger.py."""
    command = [
        args.python,
        args.script,
        str(yaml_path),
    ...
    if args.avg_window is not None:
        command.extend(["--avg-window", str(args.avg_window)])
...
def build_sbatch_command(
    ...
    """Build sbatch command wrapping one stats_trigger command."""
...
def _process_day(
    ...
    if not targets:
        logger.warning("No Trigger merged YAML files found for date {}", date_text)
        return 0, 0
...
def main() -> int:
    """Entry point for batch stats_trigger submit helper."""
    ...
    # Apply defaults for date range when neither --date nor explicit range is given
