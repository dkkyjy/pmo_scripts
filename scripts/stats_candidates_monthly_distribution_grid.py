#!/usr/bin/env python3
"""Plot monthly candidate-event-count distributions from CSV files.
...
y-axis is candidate event count, and line colors represent run_number.
"""
...
MONTH_FORMAT = "%Y-%m"
FILE_PATTERNS = "run_candidates_event_count_daily_{start}_{end}.csv"
...
def read_month_rows(csv_path: Path, month_start: dt.date) -> List[EventRow]:
    ...
    with csv_path.open("r", encoding="utf-8") as file_obj:
        ...
        for raw in reader:
            ...
            rows.append(
                EventRow(row_date, run_number, event_count, file_name, csv_path.name)
            )
...
def plot_monthly_grid(
    ...
    fig.suptitle(
        "Monthly Candidate Event Count Distribution by RUN\n"
        "(2025-07 to 2026-03)",
        fontsize=14,
        y=0.995,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Read monthly run_candidates_event_count CSV files and plot a 3x3 "
            "monthly distribution grid with run-number colors."
        )
    )
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help="Directory containing run_candidates_event_count CSV files",
    )
    parser.add_argument(
        "--start-month",
        type=parse_month,
    ...
    parser.add_argument(
        "--output-png",
        default="../Reco_Dir/run_candidates_event_count_monthly_202507_202603_grid.png",
