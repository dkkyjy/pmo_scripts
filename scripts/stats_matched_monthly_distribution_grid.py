#!/usr/bin/env python3
"""Plot monthly event-count distributions from run matched CSV files.
...
MONTH_FORMAT = "%Y-%m"
FILE_PATTERNS = "run_matched_event_count_daily_{start}_{end}.csv"
...
def fill_missing_rows_from_merged(
    ...
    """Fill missing matched rows with zero count using merged keys as reference."""
    matched_keys = {(row.date, row.run_number) for row in month_rows}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Read monthly run_matched_event_count CSV files and plot a 3x3 monthly "
    ...
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help="Directory containing run_matched_event_count CSV files",
    )
    parser.add_argument(
        "--start-month",
        type=parse_month,
    ...
    parser.add_argument(
        "--output-png",
        default="../Reco_Dir/run_matched_event_count_monthly_202507_202603_grid.png",
