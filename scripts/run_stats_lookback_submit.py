#!/usr/bin/env python3
"""Batch-submit stats_lookback.py by auto-discovering Trigger matched YAML.

This script scans Trigger matched YAML files and submits commands in the form:
    python stats/stats_lookback.py ../Reco_Dir/Trigger_YYYYMMDD_RUNxxx_matched.yaml
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    """Build command-line parser for batch stats_lookback submission."""
    parser = argparse.ArgumentParser(
        description=(
            "Batch-submit stats/stats_lookback.py by discovering "
    ...
    parser.add_argument(
        ...
        help="Python executable used to run stats_lookback.py.",
    )
    parser.add_argument(
        "--script",
        default="stats/stats_lookback.py",
        help="Path to stats_lookback.py.",
    )
    parser.add_argument(
        "--lookback",
        type=int,
        default=None,
        help="Forward --lookback to child command (default: 50).",
    )
    parser.add_argument(
        "--bins",
        type=int,
        default=None,
        help="Forward --bins to child command (default: 100).",
    )
    parser.add_argument(
        "--offset-file",
        default=None,
...
def build_child_command(args: argparse.Namespace, yaml_path: Path) -> List[str]:
    """Build child command list for stats_lookback.py."""
    command = [
        args.python,
        args.script,
        str(yaml_path),
    ...
    if args.bins is not None:
        command.extend(["--bins", str(args.bins)])
    if args.offset_file:
...
def build_sbatch_command(
    ...
    """Build sbatch command wrapping one stats_lookback command."""
...
def main() -> int:
    """Entry point for batch stats_lookback submit helper."""
