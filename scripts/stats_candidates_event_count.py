#!/usr/bin/env python3
"""Count candidate events in slope-candidates YAML files and plot daily trends.

The script scans files matching ``Trigger_*_candidates.yaml`` under
a Reco directory, counts top-level events in each file, writes a CSV,
...
FILE_PATTERN = re.compile(
    r"^Trigger_(\d{8})_RUN(\d+)_candidates\.yaml$"
)
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

OUTPUT_FILE_STEM = "run_candidates_event_count_daily"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Count per-file candidate events from "
            "Trigger_*_candidates.yaml and plot per-run daily trends."
        )
    )
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help=(
            "Directory containing candidates YAML files "
            "(default: ../Reco_Dir)"
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Single date to process (sets --start-date and --end-date "
            "automatically). e.g. 2026-01-01. Mutually exclusive with "
            "--start-date/--end-date."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "run_candidates_event_count_daily_<start-date>_<end-date>.csv"
        ),
    )
    parser.add_argument(
        "--output-png",
        ...
        help=(
            ...
            "run_candidates_event_count_daily_<start-date>_<end-date>.png"
        ),
    )
    return parser
...
def count_events_in_yaml(file_path: Path) -> int:
    """Count top-level candidate events in one candidates YAML file.
...
def collect_event_rows(
    ...
    """Collect event-count rows from all matching candidates YAML files.

    Scans ``Trigger_*_candidates.yaml`` under *reco_dir*.
    """
    rows: List[EventCountRow] = []
    candidates = sorted(reco_dir.glob("Trigger_*_candidates.yaml"))
    total_candidates = len(candidates)

    logger.info(
        "Start scanning candidates YAML files: reco_dir={}, candidates={}",
        reco_dir,
        total_candidates,
    )
...
def write_rows_csv(rows: Iterable[EventCountRow], output_csv: Path) -> None:
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as file_obj:
        ...
        writer.writerow(["date", "run_number", "event_count", "file_name"])
        for row in rows:
            writer.writerow([
...
def group_rows_by_run(
    rows: Sequence[EventCountRow],
) -> DefaultDict[int, List[EventCountRow]]:
...
def plot_rows(rows: Sequence[EventCountRow], output_png: Path) -> None:
    """Plot per-run daily candidate event counts."""
    if not rows:
    ...
    ax.set_xlabel("Date")
    ax.set_ylabel("Candidate Event Count")
    ax.set_title("Daily Candidate Event Count per RUN (candidates YAML)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    output_csv = (
        ...
        / f"{build_output_stem(OUTPUT_FILE_STEM, args.start_date, args.end_date)}.csv"
    )
    output_png = (
        Path(args.output_png)
        if args.output_png is not None
        else reco_dir
        / f"{build_output_stem(OUTPUT_FILE_STEM, args.start_date, args.end_date)}.png"
    )
