#!/usr/bin/env python3
"""Count events in SWM YAML files and plot daily trends.

The script scans files matching ``Trigger*_RUN*_SWM.yaml`` under a Reco
directory, counts top-level events in each file, writes a CSV, and generates
a time-series plot where each run number is a separate line.
"""
...
# Match both per-file SWM (Trigger_YYYYMMDDHHMMSS_RUNxxx_SWM.yaml)
# and merged SWM   (Trigger_YYYYMMDD_RUNxxx_merged_SWM.yaml).
FILE_PATTERN = re.compile(r"^Trigger_(\d{8})(?:_\d{6})?_RUN(\d+).*_SWM\.yaml$")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Count per-file events from Trigger*_RUN*_SWM.yaml "
    ...
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help="Directory containing SWM YAML files (default: ../Reco_Dir)",
    )
    parser.add_argument(
        "--date",
        type=parse_cli_date,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "run_swm_event_count_daily_<start-date>_<end-date>.csv"
        ),
    )
    parser.add_argument(
        "--output-png",
        ...
        help=(
            ...
            "run_swm_event_count_daily_<start-date>_<end-date>.png"
        ),
    )
    parser.add_argument(
        "--merged-csv",
        default=None,
        help=(
            "Optional path to a merged-*_event_count_daily.csv. "
            "If provided, missing (date, run) pairs are filled with zero counts "
            "so that gaps appear explicitly on the plot."
        ),
    )
    return parser
...
def count_events_in_yaml(file_path: Path) -> int:
    """Count top-level events in one SWM YAML file.
...
def collect_event_rows(
    ...
    """Collect event-count rows from all matching SWM YAML files."""
    rows: List[EventCountRow] = []
    candidates = sorted(reco_dir.glob("Trigger*SWM.yaml"))
    total_candidates = len(candidates)

    logger.info(
        "Start scanning SWM YAML files: reco_dir={}, candidates={}",
        reco_dir,
        total_candidates,
    )
...
def fill_missing_rows_from_merged_csv(
    ...
    """Fill missing SWM (date, run_number) pairs with zero counts."""
    merged_keys = read_event_keys_from_csv(merged_csv_path)
    swm_keys = {(row.date, row.run_number) for row in rows}
    missing_keys = sorted(merged_keys - swm_keys)

    if not missing_keys:
        return rows

    logger.info(
        "Fill missing rows from merged reference: merged_keys={} swm_keys={} missing={}",
        len(merged_keys),
        len(swm_keys),
        len(missing_keys),
    )
...
def plot_rows(rows: Sequence[EventCountRow], output_png: Path) -> None:
    ...
    ax.set_title("Daily Event Count per RUN (SWM YAML)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    output_csv = (
        ...
        / f"{build_output_stem('run_swm_event_count_daily', args.start_date, args.end_date)}.csv"
    )
    output_png = (
        Path(args.output_png)
        if args.output_png is not None
        else reco_dir
        / f"{build_output_stem('run_swm_event_count_daily', args.start_date, args.end_date)}.png"
    )
    ...
    if not rows:
        logger.warning("No matching SWM files found under {}", reco_dir)
        return 1

    # ── Fill missing from merged reference CSV ────────────────────────────
    merged_csv = (
    ...
    if merged_csv.is_file():
        logger.info("Compare with merged CSV before plotting: {}", merged_csv)
        rows = fill_missing_rows_from_merged_csv(rows, merged_csv)
    else:
        logger.warning(
