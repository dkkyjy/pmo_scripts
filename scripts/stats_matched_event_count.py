#!/usr/bin/env python3
"""Count events in RUN matched YAML files and plot daily trends.

The script scans files matching ``Trigger_YYYYMMDD_RUNxxx_matched.yaml``
...
FILE_PATTERN = re.compile(r"^Trigger_(\d{8})_RUN(\d+)_matched\.yaml$")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Count per-file events from Trigger_YYYYMMDD_RUNxxx_matched.yaml "
    ...
    parser.add_argument(
        "--reco-dir",
        default="../Reco_Dir",
        help="Directory containing matched YAML files (default: ../Reco_Dir)",
    )
    parser.add_argument(
        "--date",
        type=parse_cli_date,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "run_matched_event_count_daily_<start-date>_<end-date>.csv"
        ),
    )
    parser.add_argument(
        "--output-png",
        ...
        help=(
            ...
            "run_matched_event_count_daily_<start-date>_<end-date>.png"
        ),
    )
    return parser
...
def count_events_in_yaml(file_path: Path) -> int:
    """Count top-level events in one matched YAML file.
...
def collect_event_rows(
    ...
    """Collect event-count rows from all matching matched YAML files."""
    rows: List[EventCountRow] = []
    candidates = sorted(reco_dir.glob("Trigger_*_RUN*_matched.yaml"))
    total_candidates = len(candidates)

    logger.info(
        "Start scanning matched YAML files: reco_dir={}, candidates={}",
        reco_dir,
        total_candidates,
    )
...
def fill_missing_rows_from_merged_csv(
    ...
    """Fill missing matched (date, run_number) pairs with zero counts."""
    merged_keys = read_event_keys_from_csv(merged_csv_path)
    matched_keys = {(row.date, row.run_number) for row in rows}
    missing_keys = sorted(merged_keys - matched_keys)

    if not missing_keys:
        return rows

    logger.info(
        "Fill missing rows from merged reference: merged_keys={} matched_keys={} missing={}",
        len(merged_keys),
        len(matched_keys),
        len(missing_keys),
    )
...
def plot_rows(rows: Sequence[EventCountRow], output_png: Path) -> None:
    ...
    ax.set_title("Daily Event Count per RUN (matched YAML)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    output_csv = (
        ...
        / f"{build_output_stem('run_matched_event_count_daily', args.start_date, args.end_date)}.csv"
    )
    output_png = (
        Path(args.output_png)
        if args.output_png is not None
        else reco_dir
        / f"{build_output_stem('run_matched_event_count_daily', args.start_date, args.end_date)}.png"
    )
